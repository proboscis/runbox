...
impl std::str::FromStr for TaskRuntime {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s.to_lowercase().as_str() {
            ...
            _ => Err(format!(
                "Invalid runtime: {}. Valid: background, tmux, zellij",
                s
            )),
        }
    }
...
mod tests {
    ...
    fn test_task_lifecycle() {
        ...
        // Start
        task.mark_started(TaskHandle::Background {
            pid: 1234,
            pgid: 1234,
        });
    ...
    fn test_runtime_parsing() {
        assert_eq!(
            "background".parse::<TaskRuntime>().unwrap(),
            TaskRuntime::Background
        );
        assert_eq!(
            "bg".parse::<TaskRuntime>().unwrap(),
            TaskRuntime::Background
        );
        assert_eq!("tmux".parse::<TaskRuntime>().unwrap(), TaskRuntime::Tmux);
        assert_eq!(
            "zellij".parse::<TaskRuntime>().unwrap(),
            TaskRuntime::Zellij
        );
