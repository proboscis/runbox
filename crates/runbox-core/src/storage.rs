...
impl Storage {
    ...
    /// Get the playlist display name for a runnable.
    ///
    /// - For PlaylistItem: returns the playlist's display name
    /// - For Template/Replay: returns None
    pub fn get_runnable_playlist_name(&self, runnable: &crate::Runnable) -> Option<String> {
        match runnable {
            crate::Runnable::PlaylistItem { playlist_id, .. } => {
                self.load_playlist(playlist_id).ok().map(|p| p.name)
            }
            _ => None,
        }
    }
