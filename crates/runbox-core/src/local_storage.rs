...
impl LayeredStorage {
                ...
                for entry in fs::read_dir(&templates_dir)? {
                    ...
                    if entry.path().extension().map(|e| e == "json").unwrap_or(false) {
                        if let Ok(content) = fs::read_to_string(entry.path()) {
                            if let Ok(template) = serde_json::from_str::<crate::RunTemplate>(&content) {
    ...
    pub fn load_template(&self, template_id: &str) -> Result<(crate::RunTemplate, Scope)> {
        // Try local first
        if let Some(ref local_dir) = self.local_dir {
            let path = local_dir.join("templates").join(format!("{}.json", template_id));
            if path.exists() {
    ...
    pub fn save_template(&self, template: &crate::RunTemplate, scope: Scope) -> Result<PathBuf> {
        match scope {
            Scope::Local => {
                let local_dir = self.local_dir.as_ref()
                    .context("No local .runbox/ directory available")?;
                let path = local_dir.join("templates").join(format!("{}.json", template.template_id));
                if path.exists() {
            ...
            Scope::Global => {
                self.global_storage.save_template(template)
            }
        }
    }
    ...
    pub fn load_playlist(&self, playlist_id: &str) -> Result<(crate::Playlist, Scope)> {
        // Try local first
        if let Some(ref local_dir) = self.local_dir {
            let path = local_dir.join("playlists").join(format!("{}.json", playlist_id));
            if path.exists() {
                let content = fs::read_to_string(&path)
    ...
    pub fn save_playlist(&self, playlist: &crate::Playlist, scope: Scope) -> Result<PathBuf> {
        match scope {
            Scope::Local => {
                let local_dir = self.local_dir.as_ref()
                    .context("No local .runbox/ directory available")?;
                let path = local_dir.join("playlists").join(format!("{}.json", playlist.playlist_id));
                let json = serde_json::to_string_pretty(playlist)?;
                fs::write(&path, json)?;
                Ok(path)
            }
            Scope::Global => {
                self.global_storage.save_playlist(playlist)
            }
        }
    }
...
mod tests {
    ...
    fn test_layered_storage_local_overrides_global() {
        ...
        let storage = LayeredStorage::with_paths(
        ...
        storage.global_storage().save_template(&global_template).unwrap();
        ...
        storage.save_template(&local_template, Scope::Local).unwrap();
        ...
        let test_templates: Vec<_> = templates.iter()
    ...
    fn test_save_to_local_requires_local_dir() {
        ...
        assert!(result.unwrap_err().to_string().contains("No local .runbox/ directory"));
    }
}
